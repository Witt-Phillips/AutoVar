...
@primitive("PlusRD", None, None)
def PlusRD(args):
    x, y = args

    def estimate_result():
        x_hat, dx_hat = x.estimate()
        y_hat, dy_hat = y.estimate()
        return (x_hat + y_hat, dx_hat + dy_hat)

    return IntractableReal(estimate_result)


@primitive("PlusR", PlusRD, None)
def PlusR(args):
    x, y = args
...
def TimesRD(args):
    ...
    def estimate_result():
        x_hat, dx_hat = x.estimate()
        y_hat, dy_hat = y.estimate()
        return (x_hat * y_hat, dx_hat * y_hat + x_hat * dy_hat)

    return IntractableReal(estimate_result)


@primitive("TimesR", TimesRD, None)
def TimesR(args):
    x, y = args
...
@primitive("FlipExpectEnumD", None, None)
def FlipExpectEnumD(p):
    p, dp = p

    def integrate(f):
        def estimate_expectation():
            lossT, dlossT = f(True).estimate()
            lossF, dlossF = f(False).estimate()
            expected_value = p * lossT + (1 - p) * lossF
            deriv = p * dlossT + (1 - p) * dlossF + (lossT - lossF)
            return (expected_value, deriv)

        return IntractableReal(estimate_expectation)

    return integrate


@primitive("FlipExpectReinforceD", None, None)
def FlipExpectReinforceD(p):
    p, dp = p
...
@primitive("FlipExpectEnum", FlipExpectEnumD, None)
def FlipExpectEnum(p):
    def integrate(f):
        return interpret(PlusRD)(
            (
                interpret(TimesRD)((f(True), interpret(CastRD)(p))),
                interpret(TimesRD)((f(False), interpret(CastRD)(1 - p))),
            )
        )
...
@primitive("FlipExpectReinforce", FlipExpectReinforceD, None)
def FlipExpectReinforce(p):
    def integrate(f):
        def estimate_expectation():
            b = random.random() < p
            return f(b)
...
@primitive("FlipEnum", None, FlipExpectEnum)
def FlipEnum(p):
    def sampler():
...
@primitive("FlipReinforce", None, FlipExpectReinforce)
def FlipReinforce(p):
    def sampler():
        return random.random() < p
